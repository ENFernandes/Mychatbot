import OpenAI from 'openai';
import { Readable } from 'stream';

export type ChatMessage = {
  role: 'user' | 'assistant' | 'system' | 'developer';
  content: string;
  fileIds?: string[];
};

export interface FileContent {
  buffer: Buffer;
  filename: string;
  mimeType: string;
}

export interface UploadedFileInfo {
  providerId: string;
  filename: string;
  mimeType: string;
}

export interface StreamCallbacks {
  onChunk: (text: string) => void;
  onComplete: () => void;
  onError: (error: Error) => void;
}

export async function openaiUploadFile(params: {
  apiKey: string;
  file: FileContent;
}): Promise<UploadedFileInfo> {
  const client = new OpenAI({ apiKey: params.apiKey });
  const fileStream = Readable.from(params.file.buffer);
  (fileStream as any).name = params.file.filename;
  (fileStream as any).type = params.file.mimeType;
  (fileStream as any).size = params.file.buffer.length;

  const uploadedFile = await client.files.create({
    file: fileStream as any,
    purpose: 'assistants',
  });

  return {
    providerId: uploadedFile.id,
    filename: params.file.filename,
    mimeType: params.file.mimeType,
  };
}

export async function openaiDeleteFile(params: {
  apiKey: string;
  fileId: string;
}): Promise<boolean> {
  const client = new OpenAI({ apiKey: params.apiKey });

  try {
    await client.files.del(params.fileId);
    return true;
  } catch (error) {
    console.error('Error deleting file from OpenAI:', error);
    return false;
  }
}

function buildMessagesWithFiles(
  messages: ChatMessage[],
  imageFiles: FileContent[],
  _uploadedFiles: UploadedFileInfo[]
): any[] {
  return messages.map((m, idx) => {
    const userMessages = messages.map((msg, i) => ({ msg, i })).filter(({ msg }) => msg.role === 'user');
    const lastUserMessageIndex = userMessages.length > 0 ? userMessages[userMessages.length - 1].i : -1;
    const isLastUserMessage = m.role === 'user' && idx === lastUserMessageIndex;

    if (isLastUserMessage && imageFiles.length > 0) {
      const content: Array<
        | { type: 'text'; text: string }
        | { type: 'image_url'; image_url: { url: string; detail?: 'low' | 'high' | 'auto' } }
      > = [];

      const textContent = m.content || (imageFiles.length > 0 ? 'What do you see in this image?' : '');
      if (textContent) {
        content.push({ type: 'text', text: textContent });
      }

      if (imageFiles.length > 0) {
        for (const imageFile of imageFiles) {
          const base64Image = imageFile.buffer.toString('base64');
          const dataUrl = `data:${imageFile.mimeType};base64,${base64Image}`;
          content.push({
            type: 'image_url',
            image_url: { url: dataUrl, detail: 'auto' },
          });
        }
      }

      return {
        role: m.role === 'developer' ? 'system' : m.role,
        content: content.length > 0 ? content : m.content,
      };
    }

    return {
      role: m.role === 'developer' ? 'system' : m.role,
      content: m.content,
    };
  });
}

async function doChatCompletion(
  client: OpenAI,
  model: string,
  messages: any[],
  imageFiles: FileContent[],
  apiKey: string
): Promise<{ message: string; usage?: unknown }> {
  const isVisionModel = /gpt-4o|gpt-4-turbo|gpt-4-vision/i.test(model);
  let modelToUse = model;

  if (imageFiles.length > 0 && !isVisionModel) {
    modelToUse = 'gpt-4o';
  }

  const response = await client.chat.completions.create({
    model: modelToUse,
    messages,
    max_tokens: 4096,
    stream: false,
  });

  return { message: response.choices[0]?.message?.content || '', usage: response.usage };
}

async function doChatCompletionStream(
  client: OpenAI,
  model: string,
  messages: any[],
  imageFiles: FileContent[],
  callbacks: StreamCallbacks
): Promise<void> {
  const isVisionModel = /gpt-4o|gpt-4-turbo|gpt-4-vision/i.test(model);
  let modelToUse = model;

  if (imageFiles.length > 0 && !isVisionModel) {
    modelToUse = 'gpt-4o';
  }

  const stream = await client.chat.completions.create({
    model: modelToUse,
    messages,
    max_tokens: 4096,
    stream: true,
  });

  for await (const chunk of stream) {
    const text = chunk.choices[0]?.delta?.content || '';
    if (text) {
      callbacks.onChunk(text);
    }
  }

  callbacks.onComplete();
}

export async function openaiChat(params: {
  apiKey: string;
  model: string;
  messages: ChatMessage[];
  files?: FileContent[];
}): Promise<{ message: string; usage?: unknown }> {
  const client = new OpenAI({ apiKey: params.apiKey });

  const imageFiles: FileContent[] = [];
  const otherFiles: FileContent[] = [];

  if (params.files && params.files.length > 0) {
    for (const file of params.files) {
      if (file.mimeType.startsWith('image/')) {
        imageFiles.push(file);
      } else {
        otherFiles.push(file);
      }
    }
  }

  const uploadedFiles: UploadedFileInfo[] = [];
  if (otherFiles.length > 0) {
    for (const file of otherFiles) {
      try {
        const uploaded = await openaiUploadFile({ apiKey: params.apiKey, file });
        uploadedFiles.push(uploaded);
      } catch (err) {
        console.error('Error uploading file to OpenAI:', err);
      }
    }
  }

  const tryModels = [params.model, 'gpt-4o', 'gpt-4o-mini', 'gpt-4-turbo'].filter(Boolean);
  let lastError: any = null;

  for (const model of tryModels) {
    try {
      const messagesWithFiles = buildMessagesWithFiles(params.messages, imageFiles, uploadedFiles);

      if (imageFiles.length > 0 || uploadedFiles.length > 0) {
        const result = await doChatCompletion(client, model, messagesWithFiles, imageFiles, params.apiKey);
        
        for (const file of uploadedFiles) {
          try {
            await openaiDeleteFile({ apiKey: params.apiKey, fileId: file.providerId });
          } catch (e) {}
        }

        return result;
      }

      try {
        return await doChatCompletion(
          client,
          model,
          params.messages.map((m) => ({ role: m.role === 'developer' ? 'system' : m.role, content: m.content })),
          imageFiles,
          params.apiKey
        );
      } catch (chatError) {
        const inputText = params.messages.map((m) => `${m.role}: ${m.content}`).join('\n');
        const response = await (client as any).responses.create({
          model,
          tools: [{ type: 'web_search_preview' }],
          input: inputText,
        });

        const output = (response as any).output_text || (response as any).text || (response as any).output?.[0]?.content?.[0]?.text || '';
        return { message: output, usage: (response as any).usage };
      }
    } catch (err) {
      lastError = err;
      console.error(`Error with model ${model}:`, err);
      continue;
    }
  }

  throw lastError || new Error('Failed to get response from OpenAI');
}

export async function openaiChatStream(
  params: {
    apiKey: string;
    model: string;
    messages: ChatMessage[];
    files?: FileContent[];
  },
  callbacks: StreamCallbacks
): Promise<void> {
  const client = new OpenAI({ apiKey: params.apiKey });

  const imageFiles: FileContent[] = [];
  const otherFiles: FileContent[] = [];

  if (params.files && params.files.length > 0) {
    for (const file of params.files) {
      if (file.mimeType.startsWith('image/')) {
        imageFiles.push(file);
      } else {
        otherFiles.push(file);
      }
    }
  }

  const uploadedFiles: UploadedFileInfo[] = [];
  if (otherFiles.length > 0) {
    for (const file of otherFiles) {
      try {
        const uploaded = await openaiUploadFile({ apiKey: params.apiKey, file });
        uploadedFiles.push(uploaded);
      } catch (err) {
        console.error('Error uploading file to OpenAI:', err);
      }
    }
  }

  const tryModels = [params.model, 'gpt-4o', 'gpt-4o-mini', 'gpt-4-turbo'].filter(Boolean);

  for (const model of tryModels) {
    try {
      const messagesWithFiles = buildMessagesWithFiles(params.messages, imageFiles, uploadedFiles);

      if (imageFiles.length > 0 || uploadedFiles.length > 0) {
        await doChatCompletionStream(client, model, messagesWithFiles, imageFiles, callbacks);
        for (const file of uploadedFiles) {
          try {
            await openaiDeleteFile({ apiKey: params.apiKey, fileId: file.providerId });
          } catch (e) {}
        }
        return;
      }

      await doChatCompletionStream(
        client,
        model,
        params.messages.map((m) => ({ role: m.role === 'developer' ? 'system' : m.role, content: m.content })),
        imageFiles,
        callbacks
      );
      return;
    } catch (err) {
      console.error(`Error with model ${model}:`, err);
      continue;
    }
  }

  callbacks.onError(new Error('Failed to get response from OpenAI'));
}

export async function openaiListModels(apiKey: string): Promise<string[]> {
  try {
    const client = new OpenAI({ apiKey });
    const list = await client.models.list();
    const ids = list.data
      .map((m) => ({ id: (m as any).id as string, created: (m as any).created as number }))
      .filter((m) => /gpt|o\b|o-mini|gpt-/.test(m.id))
      .sort((a, b) => (b.created || 0) - (a.created || 0))
      .map((m) => m.id);
    return ids;
  } catch (error: any) {
    throw error;
  }
}
